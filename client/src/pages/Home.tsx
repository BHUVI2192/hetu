...
export default function Home() {
  ...
  return <div className="landing-shell"><header className="topbar landing-topbar"><Logo /><nav className="landing-nav"><a href="#method">Method</a><a href="#scenarios">Scenarios</a><a href="#capabilities">Capabilities</a></nav><div className="landing-actions"><button className="text-button"><LockKeyhole size={14} /> Sign in</button><button className="primary-button small" onClick={() => setMode("debugger")}>Open workspace <ArrowRight size={14} /></button></div></header><main><section className="hero-section"><div className="hero-copy"><div className="eyebrow-pill"><span className="live-dot" /> Causal intelligence for agent systems</div><h1>Find the error<br /><em>behind</em> the errors.</h1><p>404 AI maps agent traces into an execution graph, then proves the earliest step whose correction resolves the most downstream failures.</p><div className="hero-proof"><div><CheckCircle2 size={15} /><span>Trace-backed decisions</span></div><div><CheckCircle2 size={15} /><span>No chat. No guesswork.</span></div><div><CheckCircle2 size={15} /><span>Analysis in under 100ms</span></div></div></div><div className="hero-visual"><div className="hero-visual-header"><span><span className="live-dot" /> LIVE INVESTIGATION</span><span>TRACE · 14 SPANS</span></div><div className="mini-graph"><div className="mini-line l1" /><div className="mini-line l2" /><div className="mini-line l3" /><div className="mini-node n1 ok"><span>plan_task</span><small>Orchestrator</small></div><div className="mini-node n2 ok"><span>parse_request</span><small>DataRetrieval</small></div><div className="mini-node n3 des"><span>sql_query</span><small>DES · 0.85 risk</small></div><div className="mini-node n4 warn"><span>compute_metrics</span><small>Analysis</small></div><div className="mini-node n5 err"><span>validate_report</span><small>Reviewer</small></div></div><div className="hero-visual-footer"><span><Target size={14} /> DES identified</span><strong>data-002 <span>·</span> 86% confidence</strong></div></div></section><section className="analyze-card" id="analyze"><div className="analyze-head"><div><span className="section-eyebrow">START AN INVESTIGATION</span><h2>Bring a trace. Leave with a cause.</h2></div><div className="format-chips"><Badge tone="blue">JSON</Badge><Badge tone="neutral">OTLP</Badge><Badge tone="neutral">CSV</Badge><Badge tone="neutral">Logs</Badge></div></div><textarea value={trace} onChange={(event) => setTrace(event.target.value)} placeholder="Paste an agent trace, log file, or execution event…\n\n404 AI auto-detects OpenTelemetry, LangSmith, Langfuse, CrewAI, AutoGen, raw JSON, CSV, and plain text." /><div className="analyze-footer"><button className="upload-link"><Upload size={15} /> Upload trace file</button><span className="input-note"><FileJson size={14} /> Any format · auto-detected</span><button className="primary-button" onClick={analyze}><Play size={15} fill="currentColor" /> Analyze trace <ArrowRight size={15} /></button></div></section><section className="scenario-section" id="scenarios"><div className="section-heading"><div><span className="section-eyebrow">START WITH A SIGNAL</span><h2>Explore a known failure mode</h2></div><span className="muted">Three production-shaped traces to make the method tangible.</span></div><div className="scenario-grid">{Object.values(scenarios).map((item) => <ScenarioCard scenario={item} active={item.key === scenarioKey} key={item.key} onClick={() => { setScenarioKey(item.key); setMode("debugger"); }} />)}</div></section><section className="stats-row"><div><strong>14.2%</strong><span>LLM accuracy at DES identification</span><small>Who&When · ICML 2026</small></div><div><strong>&lt;100ms</strong><span>to analyze a 1,000-span trace</span><small>Counterfactual impact analysis</small></div><div><strong>87%</strong><span>of decisions can be corrupted</span><small>by a single compromised agent</small></div><div><strong>6</strong><span>anomaly detectors</span><small>before causal scoring begins</small></div></section><section className="method-section" id="method"><div className="method-copy"><span className="section-eyebrow">THE 404 METHOD</span><h2>Chronology tells you what happened.<br /><em>Causality tells you what to fix.</em></h2><p>404 AI runs Counterfactual Impact Analysis across the full execution graph. It scores anomalous nodes by severity, resolution potential, blast radius, and earliness — then makes the reasoning inspectable.</p><button className="link-button">Read the technical method <ArrowRight size={14} /></button></div><div className="method-steps"><div><span>01</span><strong>Observe</strong><p>Normalize any agent trace.</p></div><div><span>02</span><strong>Understand</strong><p>Build the causal graph.</p></div><div className="method-step-active"><span>03</span><strong>Prove</strong><p>Counterfactual impact scoring.</p></div><div><span>04</span><strong>Decide</strong><p>Rank engineering actions.</p></div><div><span>05</span><strong>Validate</strong><p>Replay and regression checks.</p></div><div><span>06</span><strong>Learn</strong><p>Feedback calibrates trust.</p></div></div></section><section className="capabilities-section" id="capabilities"><div className="section-heading"><div><span className="section-eyebrow">BUILT FOR SEMANTIC FAILURES</span><h2>Six signals. One decisive step.</h2></div></div><div className="capability-grid"><div><span className="cap-icon red"><CircleAlert size={16} /></span><strong>Explicit errors</strong><p>Status errors, timeouts, and failed spans.</p></div><div><span className="cap-icon amber"><RotateCcw size={16} /></span><strong>Loop detection</strong><p>Repeated calls with no convergence.</p></div><div><span className="cap-icon blue"><Eye size={16} /></span><strong>Hallucination risk</strong><p>Semantic confidence below the line.</p></div><div><span className="cap-icon purple"><Database size={16} /></span><strong>Memory poisoning</strong><p>Writes that later become trusted reads.</p></div><div><span className="cap-icon green"><GitBranch size={16} /></span><strong>Cycle detection</strong><p>Strongly connected execution paths.</p></div><div><span className="cap-icon slate"><BarChart3 size={16} /></span><strong>Token waste</strong><p>Quantified burn in detected cycles.</p></div></div></section></main><footer className="site-footer"><Logo compact /><span>404 AI · Causal Intelligence Platform</span><span className="footer-right">Built for teams shipping agent systems <Sparkles size={14} /></span></footer></div>;
}
